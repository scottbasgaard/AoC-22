...
export const solve = (lines) => {
    ...
    const freeSpace = DISK_SPACE - filesystem.get('root');
    const freeUp = REQUIRED_SPACE - freeSpace;

    const directories = [...filesystem.entries()]
        .filter(([key, size]) => key !== 'root' && size >= freeUp)
        .map(([key, size]) => size) // eslint-disable-line no-unused-vars
        .sort((a, b) => a - b);
